...
From the root of your Redmine installation, run the plugin's automated test suite with:

```bash
RAILS_ENV=test bundle exec rspec plugins/bless_this_redmine_sso
```

All specs live inside the plugin's `spec/` directory and require the Rails test environment.
