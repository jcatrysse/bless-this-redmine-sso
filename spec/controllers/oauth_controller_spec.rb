...
if defined?(OauthController) && defined?(Setting)
  RSpec.describe OauthController, type: :controller do
    ...
    describe 'GET #callback' do
      ...
      it 'redirects to the stored back_url even when the session is reset' do
        session[:oauth_state] = 'expected'
        session[:oauth_back_url] = '/issues/48'

        allow(controller).to receive(:exchange_code_for_token)
          .and_return('access_token' => 'token', 'id_token' => 'jwt')
        allow(controller).to receive(:verify_id_token).and_return('sub' => 'user-1')
        allow(controller).to receive(:get_user_info).and_return('sub' => 'user-1')
        ...
        allow(controller).to receive(:logged_user=) do |value|
          captured_logged_user = value
          controller.reset_session
        end

        get :callback, params: { state: 'expected', code: 'auth-code' }

        expect(response).to redirect_to('/issues/48')
        expect(session[:oauth_logged_in]).to eq(true)
